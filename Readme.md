...
1. *Lambda Tasks* - to keep with AWS standards, function handlers now default to `index` instead of the v1 required `lambda`.
...
+ Objects in the *tasks* array are of 3 types
    ...
    ```
        ...
        + create a Lambda function named *ld_SomeGoodName_nameYourFunction*, using the role noted by the role arn, the 1024 GB memory configuration, a timeout of 5 seconds, and running on NodeJS 6.10
