...
+ Objects in the *tasks* array are of 3 types
    ...
    ```
    {
        ...
        "functions": [
            { "name": "nameYourFunction", "source": "/path/to/function.js", "iamRoleArn":"arn:aws:iam::1234567890:role/yourRoleNameHere", "memorySize": 1024, "timeout": 5 }
        ]
    }
    ```
        ...
        + create a Lambda function named *ld_SomeGoodName_nameYourFunction*, using the role noted by the role arn, the 1024 GB memory configuration and a timeout of 5 seconds
            + if either *memorySize* or *timeout* is not included, the default configured with `default` will be used
