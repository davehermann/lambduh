 ...
 * @param {Number} [maxRetries=12] - The maximum number of times to retry the function
 ...
 */
function retryable(awsScope, method, configuration, reason, failureMessage, retryInterval = 5, maxRetries = 12, retryCount, creationData) {
    if (retryCount === undefined) {
        retryCount = 0;

        Warn(`This will retry every ${retryInterval} seconds, up to ${maxRetries}, due to delays in ${reason}`);
    }
